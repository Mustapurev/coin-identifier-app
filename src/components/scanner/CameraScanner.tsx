...
export function CameraScanner() {
  ...
  if (processing) {
    return <FullPageSpinner message="Identifying coin..." />;
  }
  ...
  if (error) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] p-8 text-center">
        ...
        <button
          onClick={startCamera}
          className="px-6 py-2.5 bg-brand-600 hover:bg-brand-700 text-white rounded-lg font-medium"
        >
          Try Again
        </button>
      </div>
  ...
  if (!isActive) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] p-8 text-center">
        <div className="w-24 h-24 mb-6 rounded-full bg-brand-800/50 flex items-center justify-center">
          <span className="text-4xl">📷</span>
        </div>
        <h2 className="text-xl font-bold text-white mb-2">Coin Scanner</h2>
        <p className="text-gray-400 mb-6 max-w-sm">
          Point your camera at a coin to identify it instantly.
        </p>
        <button
          onClick={startCamera}
          className="px-8 py-3 bg-gradient-to-r from-brand-600 to-brand-700 hover:from-brand-500 hover:to-brand-600 text-white rounded-xl font-semibold text-lg shadow-lg shadow-brand-600/25 active:scale-95"
        >
          Open Camera
        </button>
    ...
    <div className="relative min-h-[60vh]">
      <div className="relative w-full max-w-md mx-auto rounded-2xl overflow-hidden bg-black">
        ...
        <div className="absolute inset-0 pointer-events-none">
          ...
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
            <div className="w-20 h-20 rounded-full border-2 border-gold-400/50" />
          </div>
        </div>

        <div className="absolute top-4 left-1/2 -translate-x-1/2 bg-black/60 backdrop-blur-sm px-4 py-1.5 rounded-full">
          <p className="text-white text-xs font-medium">Center the coin in the frame</p>
        </div>
      </div>
      ...
      <div className="flex justify-center mt-6">
        <button
          ...
          disabled={isScanning}
          className="w-20 h-20 rounded-full bg-white border-4 border-brand-800 hover:scale-105 active:scale-95 transition-all shadow-xl disabled:opacity-50 flex items-center justify-center"
        >
          <div className="w-16 h-16 rounded-full bg-white border-2 border-brand-300" />
        </button>
      </div>
